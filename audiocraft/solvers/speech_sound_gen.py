...
from ..data.speech_sound_dataset import SpeechSoundDataset, MusicInfo, AudioInfo
...
class SpeechSoundGenSolver(base.StandardSolver):
    """Solver for MusicGen training task.

    Used in: https://arxiv.org/abs/2306.05284
    """
    DATASET_TYPE: builders.DatasetType = builders.DatasetType.SPEECH_SOUND
        ...
        if self._cached_batch_loader is None or self.current_stage != "train":
            ...
            for info in infos:
                if isinstance(info, MusicInfo):
                    ...
                    assert isinstance(dataset, SpeechSoundDataset), type(dataset)
